...
if ($Install) {
    ...
    $modDataDir = Join-Path $modsDir 'PlayerList'
...
if ($Package) {
    ...
    $zip = Join-Path $PSScriptRoot 'PlayerList.zip'

    if (Test-Path $stage) { Remove-Item $stage -Recurse -Force }
    New-Item -ItemType Directory -Force -Path (Join-Path $stage 'PlayerList') | Out-Null
    Copy-Item $dll -Destination $stage -Force
    Copy-Item (Join-Path $PSScriptRoot 'manifest.json') -Destination (Join-Path $stage 'PlayerList') -Force
